...
class Memorization_Add_Title(ui.view):
    """
    ...
    def __init__(self):
        super().__init__()
        self.title = ui.TextInput(label="タイトル",style=discord.TextStyle.short)
    ...
    async def on_submit(self,interaction:discord.Interaction):
        ...
        await interaction.response.send_message(embed=embed,ephemeral=True,view=Memorization_Add_Discord_View(title))

class Memorization_Add_Discord_Ui(ui.View):
    """
...
class Memorization_Add_Discord_Ui_Select_Edit_Count(discord.ui.SelectMenu):
    """
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        """
        embed = discord.Embed(title="問題追加", description=f"現在の選択問題設定個数:{int(self.values[0])}", color=0x00ff00, view=Memorization_Add_Discord_View(self.title))
        await interaction.response.edit_message(embed=embed, view=Memorization_Add_Discord_Ui_Select(self.title, int(self.values[0])))


class Memorization_Add_Discord_Ui_Select(ui.View):
    """
...
class Memorization_question_Discord_Select(discord.ui.SelectMenu):
    """
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.mode == "delete":
            ...
            await interaction.response.send_message("問題を選択してください", ephemeral=True, view=Memorization_Edit_Discord_Select(lists, title))

class Memorization_delete_Discord_Select(discord.ui.SelectMenu):
    """
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title="問題追加", color=0x00ff00, view=Memorization_Add_Discord_View(self.title))
        await interaction.response.edit_message(embed=embed, view=Memorization_Add_Discord_View(self.title))

class Memorization_Edit_Discord_Select(discord.ui.SelectMenu):
    """
...
class Memorization_Edit_Discord_Ui_Mode_0(ui.View):
    """
...
class Memorization_Edit_Discord_Ui_Mode_1(ui.View):
    """
    A class representing a user interface for adding a memorization mission with multiple choice options in a Discord bot.
...
class Memorization_Add_Discord_Button(discord.ui.Button):
    """
    ...
    @discord.ui.button(label="問題を追加", style=discord.ButtonStyle.red)
    ...
    @discord.ui.button(label="選択問題を追加", style=discord.ButtonStyle.red)
    async def select_add(self, button, interaction:discord.Interaction):
        await interaction.response.send_modal(Memorization_Add_Discord_Ui_Select(self.title, self.base_count))

    @discord.ui.button(label="選択数変更", style=discord.ButtonStyle.red)
    async def count(self, button, interaction:discord.Interaction):
        await interaction.response.edit_message("選択数を選択してください",view=Memorization_Add_Discord_Ui_Select_Edit_Count(self.title))
    ...
    async def delete(self, button, interaction:discord.Interaction):
        ...
        await interaction.response.send_message("削除する問題を選択してください",view=Memorization_delete_Discord_Select(lists,self.title),ephemeral=True)
    ...
    async def edit(self, button, interaction:discord.Interaction):
        ...
        await interaction.response.send_message("編集する問題を選択してください",view=Memorization_Edit_Discord_Select(lists,self.title),ephemeral=True)
...
class Memorization_Add_Discord_View(discord.ui.View):
    def __init__(self,title):
        super().__init__()
        self.add_item(Memorization_Add_Discord_Button(title))
...
class Memorization_maker_main_Cog(commands.Cog):
    ...
    async def memorization_add(self, interaction: discord.Interaction):
        ...
        Returns:
            None
        """
        await interaction.response.send_message("タイトルを入力してください", ephemeral=True, view=Memorization_Add_Title())
    ...
    async def memorization_delete(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message("削除する問題を選択してください", view=Memorization_question_Discord_Select(lists, "delete"), ephemeral=True)
...
async def setup(bot):
    bot.add_cog(Memorization_maker_main_Cog(bot))
    print("[SystemLog] Memorization_maker_main_Cog：ロード完了")